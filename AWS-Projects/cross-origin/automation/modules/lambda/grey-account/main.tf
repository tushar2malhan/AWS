# Create the IAM role
resource "aws_iam_role" "iam_role_grey" {
...
# Cloud watch full access to iam role
resource "aws_iam_role_policy_attachment" "iam_role_grey_cloudwatch" {
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchFullAccess"
  role       = aws_iam_role.iam_role_grey.name
}
    ...
    "Statement": [
      {
        "Effect": "Allow",
        "Action": "sts:AssumeRole",
        "Resource": "arn:aws:iam::${var.blue_account_id}:role/Paysafe-upf-dataops-access-for-grey"
      }
    ]
  })
}
...
resource "aws_iam_role_policy_attachment" "iam_role_grey_custom_kinesis" {
  policy_arn = aws_iam_policy.custom_policy_kinesis.arn
  role       = aws_iam_role.iam_role_grey.name
}

resource "aws_iam_role_policy_attachment" "iam_role_grey_custom_assume_role" {
...
data "archive_file" "lambda_code" {
  ...
  depends_on  = [aws_iam_role_policy_attachment.iam_role_grey_cloudwatch, aws_iam_role_policy_attachment.iam_role_grey_custom_kinesis, aws_iam_role_policy_attachment.iam_role_grey_custom_assume_role]
}

resource "aws_lambda_function" "lambda_function" {
  function_name    = var.function_name
  role             = aws_iam_role.iam_role_grey.arn
  runtime          = var.runtime
  handler          = var.handler
  ...
  memory_size      = 128
  publish          = true
  depends_on       = [aws_iam_role_policy_attachment.iam_role_grey_cloudwatch]

  # environment variables
  environment{
    variables = {
      kinesis_stream_name = "Paysafe-upf-dataops-Emr_Kinesis_details_Stream"
      role_name = "Paysafe-upf-dataops-access-for-grey"
...
# event bridge rule for grey acc
